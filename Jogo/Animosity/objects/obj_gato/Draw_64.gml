if (dialogue_active) {
    ...
    // Texto principal com quebra de linha usando \n
    draw_set_color(c_white);
    draw_text_ext(box_x1 + margin + 600, box_y1 + margin, node.speaker + " " + node.text, -1, box_x2 - box_x1 - margin);
    ...
    if (array_length(node.options) == 0) {
        draw_set_color(c_black);
        draw_text(box_x1 + margin + 500, box_y2 + margin, "(Pressione ESPAÇO para salvar o jogo)");
    }
}
